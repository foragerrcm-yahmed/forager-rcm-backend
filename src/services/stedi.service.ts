 ...
 */
async function stediRequestTo<T>(
  baseUrl: string,
  method: 'GET' | 'POST' | 'PUT',
  path: string,
  body?: object
...
function stediEligibilityRequest<T>(method: 'GET' | 'POST' | 'PUT', path: string, body?: object): Promise<T> {
  return stediRequestTo<T>(STEDI_ELIGIBILITY_URL, method, path, body);
}

/** Claims / status requests — uses STEDI_CLAIMS_URL (mock in dev, real in prod). */
function stediClaimsRequest<T>(method: 'GET' | 'POST' | 'PUT', path: string, body?: object): Promise<T> {
  return stediRequestTo<T>(STEDI_CLAIMS_URL, method, path, body);
}

/** Payer search and misc — always uses real Stedi. */
function stediRequest<T>(method: 'GET' | 'POST' | 'PUT', path: string, body?: object): Promise<T> {
  return stediRequestTo<T>(STEDI_ELIGIBILITY_URL, method, path, body);
}
...
export async function checkEligibility(
  ...
  try {
    rawResponse = await stediEligibilityRequest<any>('POST', '/change/medicalnetwork/eligibility/v3', requestBody);
  } catch (e: any) {
...
export async function processEra835(payload: any, organizationId: string) {
  ...
  const eraPaymentInfos: Array<{ pi: any; financialInfo: any }> = [];
  ...
  for (const { pi, financialInfo } of eraPaymentInfos) {
    ...
    const patientControlNumber: string = cpi.patientControlNumber ?? '';
    ...
    const servicesByCode = new Map<string, typeof claim.services[0]>();
    for (const svc of claim.services) {
      if (svc.cptCodeCode) servicesByCode.set(svc.cptCodeCode, svc);
    }

    // Per-service-line comparison: sum up contracted rates and paid amounts
    const serviceLines: any[] = pi.serviceLines ?? [];
    ...
    for (const sl of serviceLines) {
      ...
      const cptCode: string = spi.adjudicatedProcedureCode ?? '';
    ...
    const claimAdjs = (pi.claimAdjustments ?? []).map((adj: any) => ({
    ...
    const totalAdjustment = claimAdjs.reduce((sum: number, a: any) => sum + (a.amount ?? 0), 0);

    // Collect remark codes from service-line adjustments
    const remarkCodes: string[] = [];
    for (const sl of serviceLines) {
      for (const sa of sl.serviceAdjustments ?? []) {
        if (sa.adjustmentReasonCode1) remarkCodes.push(sa.adjustmentReasonCode1);
        if (sa.adjustmentReasonCode2) remarkCodes.push(sa.adjustmentReasonCode2);
      }
    }
    ...
    await prisma.claim.update({
      ...
      data: {
        ...
        denialCode:
          (newStatus === 'Denied' || newStatus === 'DeniedPatientResponsibility') && primaryAdj
            ? `${primaryAdj.groupCode}-${primaryAdj.reasonCode}`
            : claim.denialCode,
      },
    });
  ...
  for (const eraClaim of legacyClaims) {
    ...
    const newStatus: ClaimStatus = resolveStatus(claimPaymentAmount, benchmark, patientResponsibilityAmount);
    ...
    await prisma.claim.update({
      ...
      data: {
        ...
        allowedAmount: eraClaim.allowedAmount ?? null,
        patientResponsibility: patientResponsibilityAmount > 0 ? patientResponsibilityAmount : null,
        adjustmentAmount: adjustments.reduce((sum: number, a: any) => sum + (a.amount ?? 0), 0),
        denialCode:
          (newStatus === 'Denied' || newStatus === 'DeniedPatientResponsibility') && primaryAdj
            ? `${primaryAdj.groupCode}-${primaryAdj.reasonCode}`
            : claim.denialCode,
