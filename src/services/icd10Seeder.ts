...
const COMMON_ICD10_CODES: { code: string; description: string; category: string }[] = [
...
export async function seedIcd10Codes(organizationId?: string): Promise<{
  created: number;
  skipped: number;
  errors: number;
  organizationsSeeded: string[];
}> {
  let orgs: { id: string }[];
