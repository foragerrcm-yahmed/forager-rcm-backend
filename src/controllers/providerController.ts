...
export const createProvider = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (npi) {
      const existingProvider = await prisma.provider.findFirst({ where: { npi, organizationId: organizationId as string } });
      if (existingProvider) {
...
export const updateProvider = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existingProvider = await prisma.provider.findUnique({ where: { id: id as string } });
    if (!existingProvider || existingProvider.organizationId !== (req.user?.organizationId as string | undefined)) {
...
export const deleteProvider = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const dependentRecords = await prisma.$transaction([
      prisma.visit.count({ where: { providerId: id as string } }),
      prisma.claim.count({ where: { providerId: id as string } }),
    ]);
    ...
    await prisma.provider.delete({ where: { id: id as string } });
    res.status(204).send();
  } catch (error) {
