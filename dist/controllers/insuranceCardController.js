...
exports.parseInsuranceCard = exports.parseCardForNewPatient = exports.deleteInsuranceCard = exports.uploadInsuranceCard = void 0;
...
function detectLayout(lines) {
    ...
    for (const line of lines) {
        if (LABELED_LABEL_RE.test(line) && line.length < 60)
            labeledScore++;
        if (DIGITAL_LABEL_RE.test(line.trim()))
            digitalScore += 2;
        if (INLINE_RE.test(line))
            inlineScore++;
    }
    if (digitalScore >= 2 && digitalScore >= labeledScore)
        return 'DIGITAL';
    if (inlineScore > labeledScore)
        return 'INLINE';
...
function looksLikeName(line) {
    ...
    for (const t of tokens) {
        if (!/^[A-Za-z][A-Za-z'\-\.]*$/.test(t))
            return false;
        if (NON_NAME_WORDS.has(t.toLowerCase()))
            return false;
    }
    return true;
}
function splitNameParts(raw) {
    ...
    if (tokens.length === 2)
        return { firstName: tokens[0], middleName: null, lastName: tokens[1], suffix };
...
function extractLabeled(lines, fullText, result) {
    /**
     ...
     */
    function findAfterLabel(labelPattern) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
            if (labelPattern.test(lines[i]) && lines[i].length < 60) {
                ...
                const nextIsLabel = next.length < 40 && (/:\s*$/.test(next) ||
    ...
    function findDollar(labelPattern) {
        const wrapped = new RegExp('(?:' + labelPattern.source + ')', 'i');
    ...
    if (!result.memberName) {
        ...
        if (raw) {
            ...
            if (cleaned.length >= 3 && /[A-Za-z]{2}/.test(cleaned)) {
                ...
                if (!result.firstName)
                    result.firstName = parts.firstName ? toTitleCase(parts.firstName) : null;
                if (!result.middleName)
                    result.middleName = parts.middleName ? toTitleCase(parts.middleName) : null;
                if (!result.lastName)
                    result.lastName = parts.lastName ? toTitleCase(parts.lastName) : null;
                if (!result.suffix)
                    result.suffix = parts.suffix;
            }
        }
    }
    // Plan Name
    ...
    if (!result.groupNumber) {
        const m = /rx\s*grp[:\s]+([A-Z0-9]{2,12})/i.exec(fullText);
        if (m)
            result.groupNumber = m[1];
    }
    // Copays
    if (!result.copayPrimary)
        result.copayPrimary = findDollar(/\b(?:primary\s*care|pcp|office\s*visit)\b/i) ?? findDollar(/\bcopay\b/i);
    if (!result.copaySpecialist)
        result.copaySpecialist = findDollar(/\bspecialist\b/i);
    if (!result.copayUrgentCare)
        result.copayUrgentCare = findDollar(/\burgent\s*care\b/i);
    if (!result.copayER)
        result.copayER = findDollar(/\b(?:emergency\s*room|er\s*copay|emergency\s*care)\b/i);
    // Deductible / OOP
    if (!result.deductible)
        result.deductible = findDollar(/\bdeductible\b/i);
    if (!result.oopMax)
        result.oopMax = findDollar(/\bout[\s\-]*of[\s\-]*pocket\s*(?:max(?:imum)?)?|\boop\s*max\b/i);
    // Effective Date
    if (!result.effectiveDate) {
        const m = /(?:effective\s+date|eff\.?\s*date|coverage\s*date)[:\s]+(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/i.exec(fullText);
        if (m)
            result.effectiveDate = parseIsoDate(m[1]);
    }
    // Phone
    if (!result.insurerPhone) {
        const m = /(?:1[\s\-]?)?\(?\d{3}\)?[\s\-\.]\d{3}[\s\-\.]\d{4}/.exec(fullText);
        if (m)
            result.insurerPhone = m[0].trim();
    }
}
...
function extractDigital(lines, fullText, result) {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
        ...
        if (labelParts.length === 1) {
            ...
            while (wi < words.length) {
                ...
                if (!matched)
                    wi++; // skip unrecognised word
            }
            if (matchedLabels.length > 1)
                labelParts = matchedLabels;
        }
    ...
    if (!result.memberName) {
        for (const line of lines) {
            if (looksLikeName(line)) {
                ...
                if (!result.firstName)
                    result.firstName = parts.firstName;
                if (!result.middleName)
                    result.middleName = parts.middleName;
                if (!result.lastName)
                    result.lastName = parts.lastName;
                if (!result.suffix)
                    result.suffix = parts.suffix;
                break;
            }
...
function applyDigitalField(result, field, rawValue) {
    if (field === 'rxGrp_tmp') {
        // RxGrp: store in rxPcn if not set, also groupNumber
        if (!result.rxPcn)
            result.rxPcn = rawValue.replace(/[^A-Z0-9\-]/gi, '');
        if (!result.groupNumber)
            result.groupNumber = rawValue.replace(/[^A-Z0-9\-]/gi, '');
        return;
    }
...
function extractInline(lines, fullText, result) {
    function findInline(labelPattern) {
        const wrapped = new RegExp('(?:' + labelPattern.source + ')', 'i');
        const combined = new RegExp(wrapped.source + '[:\\s#\\.]+([A-Z0-9][\\w\\-/ ]{0,40})', 'i');
        const m = combined.exec(fullText);
        return m ? m[1].trim() : null;
    }
    function findDollarInline(labelPattern) {
        const wrapped = new RegExp('(?:' + labelPattern.source + ')', 'i');
        const combined = new RegExp(wrapped.source + '[:\\s]+\\$?\\s*([\\d,]+(?:\\.\\d{2})?)', 'i');
        const m = combined.exec(fullText);
    ...
    if (!result.copayPrimary)
        result.copayPrimary = findDollarInline(/\b(?:primary\s*care|pcp|office\s*visit)\b/i) ?? findDollarInline(/\bcopay\b/i);
    if (!result.copaySpecialist)
        result.copaySpecialist = findDollarInline(/\bspecialist\b/i);
    if (!result.copayUrgentCare)
        result.copayUrgentCare = findDollarInline(/\burgent\s*care\b/i);
    if (!result.copayER)
        result.copayER = findDollarInline(/\b(?:emergency\s*room|er\s*copay|emergency\s*care)\b/i);
    if (!result.deductible)
        result.deductible = findDollarInline(/\bdeductible\b/i);
    if (!result.oopMax)
        result.oopMax = findDollarInline(/\bout[\s\-]*of[\s\-]*pocket\s*(?:max(?:imum)?)?|\boop\s*max\b/i);
    if (!result.effectiveDate) {
        const m = /(?:effective\s*(?:date)?|eff\.?\s*date|coverage\s*date)[:\s]+(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/i.exec(fullText);
        if (m)
            result.effectiveDate = parseIsoDate(m[1]);
...
function extractFields(text) {
    ...
    if (!result.coverageType) {
        ...
        if (m)
            result.coverageType = m[1].toUpperCase();
    }
 ...
 *   2. TOMMY DOE - Child
 */
function extractDependents(fullText, lines, primaryName) {
    const deps = [];
    ...
    if (sectionMatch && sectionMatch[1]) {
        ...
        for (const entry of entries) {
            ...
            if (nameParts.length >= 2) {
                ...
                if (primaryName && namePart.toLowerCase() === primaryName.toLowerCase())
                    continue;
...
// ─── Upload card ──────────────────────────────────────────────────────────────
const uploadInsuranceCard = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const file = req.file;
        if (!file) {
            (0, errors_1.sendError)(res, 400, (0, errors_1.validationError)('INSURANCE_CARD'), 'No image file provided');
            return;
        }
        if (file.size > MAX_SIZE) {
            (0, errors_1.sendError)(res, 400, (0, errors_1.validationError)('INSURANCE_CARD'), 'Image exceeds 10 MB limit');
            return;
        }
        const ext = path.extname(file.originalname).toLowerCase().slice(1);
        if (!ALLOWED_IMAGE_TYPES.includes(ext)) {
            (0, errors_1.sendError)(res, 400, (0, errors_1.validationError)('INSURANCE_CARD'), `File type .${ext} is not supported. Use JPG, PNG, or WebP.`);
            return;
        }
        ...
        (0, prismaErrors_1.handlePrismaError)(res, error, 'INSURANCE_CARD');
    }
};
exports.uploadInsuranceCard = uploadInsuranceCard;
// ─── Delete card ──────────────────────────────────────────────────────────────
const deleteInsuranceCard = async (req, res) => {
    try {
        const { id } = req.params;
        const policy = await prisma.patientInsurance.findUnique({ where: { id } });
        if (!policy) {
...
exports.deleteInsuranceCard = deleteInsuranceCard;
 ...
 */
const parseCardForNewPatient = async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
            (0, errors_1.sendError)(res, 400, (0, errors_1.validationError)('INSURANCE_CARD'), 'No image file provided');
        ...
        (0, errors_1.sendError)(res, 500, 'CARD_PARSE_FAILED', error?.message ?? 'Failed to parse insurance card');
    }
};
exports.parseCardForNewPatient = parseCardForNewPatient;
// ─── Parse card (Tesseract OCR + regex extraction) ────────────────────────────
const parseInsuranceCard = async (req, res) => {
    try {
        const { id } = req.params;
        const policy = await prisma.patientInsurance.findUnique({ where: { id } });
        if (!policy) {
        ...
        const ext = path.extname(file.originalname).toLowerCase().slice(1);
        if (!ALLOWED_IMAGE_TYPES.includes(ext)) {
            (0, errors_1.sendError)(res, 400, (0, errors_1.validationError)('INSURANCE_CARD'), `File type .${ext} is not supported.`);
            return;
        }
        // Run Tesseract OCR
        const rawText = await runOCR(file.buffer);
...
exports.parseInsuranceCard = parseInsuranceCard;
