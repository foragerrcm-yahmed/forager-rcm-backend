...
exports.resolveClaimStatus = resolveClaimStatus;
exports.recalculateClaimStatus = recalculateClaimStatus;
exports.applyClaimStatusRecalculation = applyClaimStatusRecalculation;
...
// ─── Status resolution ────────────────────────────────────────────────────────
function resolveClaimStatus(totalReceived, benchmark, patientResponsibility) {
    if (totalReceived === 0) {
 ...
 */
async function recalculateClaimStatus(claimId) {
    ...
    if (!claim)
        return null;
    // ── Self-pay detection ────────────────────────────────────────────────────
    const payorCategory = claim.payor?.payorCategory ?? '';
    ...
    const totalReceived = claim.paymentPostings.reduce((sum, p) => sum + Number(p.paidAmount), 0);
 ...
 */
async function applyClaimStatusRecalculation(claimId, updatedById, now) {
