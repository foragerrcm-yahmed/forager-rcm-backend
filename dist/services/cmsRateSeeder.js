...
function parseCmsRvuFile(filePath) {
    ...
    for (let i = 10; i < lines.length; i++) {
        ...
        if (mod !== '')
            continue;
        if (!hcpcs.match(/^\d{5}[A-Z0-9]?$/))
            continue; // CPT codes are 5 digits (optionally + letter)
...
async function seedCmsRates(targetOrganizationId) {
